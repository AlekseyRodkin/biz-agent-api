-- =============================================================================
-- AiShift: Supabase Schema Migration
-- Version: 0001_init (fixed v0.1.1)
-- Description: База знаний для обучающего AI-агента (курс + память + прогресс)
-- =============================================================================
...
-- 2) course_lectures — метаданные лекций
-- -----------------------------------------------------------------------------
...
create index if not exists idx_course_lectures_module_day_order
  on course_lectures (module, day, lecture_order);

-- 3) course_chunks — чанки курса с эмбеддингами
-- -----------------------------------------------------------------------------
...
create index if not exists idx_course_chunks_lecture_seq
  on course_chunks (lecture_id, sequence_order);

create index if not exists idx_course_chunks_module_day
  on course_chunks (module, day);

create index if not exists idx_course_chunks_embedding_hnsw
  on course_chunks using hnsw (embedding vector_cosine_ops);

-- 4) company_memory — память компании (решения пользователя)
-- -----------------------------------------------------------------------------
...
create index if not exists idx_company_memory_user_status
...
-- 5) user_progress — прогресс прохождения курса
-- -----------------------------------------------------------------------------
...
-- 6) RPC: match_course_chunks — поиск по курсу (cosine similarity)
-- -----------------------------------------------------------------------------
...
-- 7) RPC: match_company_memory — поиск по памяти компании
-- -----------------------------------------------------------------------------
